...
В личном чате с ботом владелец может одним сообщением установить endpoint AI
Tunnel:
...
После этого задайте модель и ключ API:
...
Ключ создаётся в [панели AI Tunnel](https://aitunnel.ru/panel/keys). Проект
тестировался с AI Tunnel; регистрация по [этой ссылке](https://aitunnel.ru?r=43877)
вежливо поддержит его дальнейшую разработку.

Для генерации изображений задайте отдельную модель:

```text
/image model set <image-model>
```

Текстовые запросы идут через OpenAI-compatible Chat Completions, а изображения
через `/images/generations`. При ответе Hermes с изображением бот отправляет его
как XMPP OOB-вложение. В диалоге с OMEMO такая отправка намеренно блокируется,
чтобы не понижать шифрование до обычного XMPP-сообщения.

### Размещение рядом с XMPP-сервером

Если Hermes и XMPP-сервер работают на одной машине, специальных изменений не
нужно: Hermes остаётся отдельным непривилегированным пользователем, а конфигурация
ejabberd и его данные не изменяются. Чтобы не выводить соединение за пределы
машины, можно указать `XMPP_HOST=127.0.0.1`, если ejabberd принимает direct TLS
на loopback-порту `5223`. TLS отключать не следует: сертификат и SNI по-прежнему
проверяются по домену JID. Если локального listener нет, оставьте обычный домен
XMPP-сервера.
