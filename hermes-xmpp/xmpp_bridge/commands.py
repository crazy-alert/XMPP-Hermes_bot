...
_HELP_TEXT = (
    "Настройка AI:\n"
    "/setaitunnel — установить endpoint AI Tunnel\n"
    "/model set <model> — выбрать текстовую модель\n"
    "/endpoint set <url> — установить endpoint другого провайдера\n"
    f"/token set <token> — сохранить API-ключ: {AITUNNEL_KEYS_URL}\n"
    "/image model set <model> — выбрать модель изображений\n"
    "/image status — показать модель изображений\n\n"
    "Доступ и состояние:\n"
    "/status, /config — показать текущую конфигурацию без ключа\n"
